...
#? Can this be deleted as deprecated?
# @all_dets
# def modify_all_headers(det_num, fits_dir, wavelength, fov_pixels=364):

#     detector = "SCA{:02}".format(det_num)

#     outfile = f"{wfi.name}_{wfi.filter}_fovp{fov_pixels}_{wavelength}_{detector}".lower() #_det.fits is added automatically

#     add_git_sha_header(os.path.join(fits_dir, outfile))

#     return 0

# def add_git_sha_header(filepath, ext=0, **kwargs):

#     gitsha = subprocess.check_output(["git", "rev-parse", "HEAD"]).decode("ascii").strip()

#     file = fits.open(filepath)
    
#     file[ext].header["gitsha"] = gitsha
#     file.writeto(filepath, overwrite=True)
#     file.close()

#     return 0

# def check_gitsha(filepath, ext=0):
...
#     try:
#         if file[ext].header["gitsha"] == gitsha:
#             print("The versions match.")
#             return 0
#         else:
#             print("The versions do not match. Try again, or generate new grids.")
#             return 1
#     except KeyError as msg:
#         print(f"\n"
#               f"The file provided does not contain a git SHA version number in the specified extension:\n"
#               f"filepath: {filepath}\n"
#               f"extension: {ext}\n")
#         raise msg
    

# put args and kwargs into a dict, add the stpsf version to that dict, and hash
# add all args/kwargs into the header
# set a kwargs flag to True/False to flag use of kwargs